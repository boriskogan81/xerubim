<template>
  <q-card style="min-width: 30vw">
    <q-card v-if="contractStore.selectedContract">
      ...
      <q-card-actions>
        <q-btn v-if="contractStore.selectedContract.data[6] === 'open'" flat
               @click="fulfillmentFields = !fulfillmentFields">Fill Contract
        </q-btn>
        <q-btn
          v-if="contractStore.selectedContract.data[6] === 'proposed' && isCustomer(contractStore.selectedContract)"
          flat @click="acceptProposal">Accept Proposal
        </q-btn>
        <q-btn
          v-if="contractStore.selectedContract.data[6] === 'proposed' && isCustomer(contractStore.selectedContract)"
          flat @click="disputeProposal">Dispute Proposal
        </q-btn>
      ...
      <q-card-section v-if="contractStore.selectedContract.data[6] === 'open'" class="q-pa-lg">
        <q-uploader
          ...
          :form-fields="[
            {name: 'notes', value: fulfillmentNotes},
            {name: 'contract', value: contractStore.selectedContract}
            ]"
  ...
  setup(props, context) {
    ...
    const onFileUpload = async (info) => {
      ...
      try {
        ...
        await web3Store.proposeMedia(contractStore.selectedContract.address, cids);
        await contractStore.updateQuery({...contractStore.contractQuery});
        dismiss();
        context.emit('closeContractDialog');
      } catch (e) {
